...
/**
 * IndexedDB wrapper that allows an interface to interact with the 
 * Movies and People stores.
 * @module IndexedDB The IndexedDB wrapper.
 */
  ...
  const dbPromise = idb.openDB("movies-db", 1, {
    upgrade(upgradeDb) {
      ...
      if (!upgradeDb.objectStoreNames.contains("movies")) {
        const moviesOS = upgradeDb.createObjectStore("movies", {
          keyPath: "imdbID",
        });
      ...
      if (!upgradeDb.objectStoreNames.contains("people")) {
        const peopleOS = upgradeDb.createObjectStore("people", {
          keyPath: "id",
        });
